...
description: Worked examples — read-before-edit, AST search, ask_user, PowerShell-safe bash, sg rewrite. Load when you want a concrete tool-call pattern.
---

# Tool-Use Examples

Worked examples of the correct tool-call shape for common coding-agent tasks.
The examples are illustrative: live workspace state always comes from
`<workspace_context>` and tool results, not from these samples.
