...
import type { LspDiagnostic, LspLocation } from '../lsp/lspSession.js';
import {
  lspChangeDocument,
  lspCloseDocument,
  lspDefinition,
...
export function registerLspIpc(): void {
  wrapIpcHandler(
    ...
    async (
      ...
      assertString('lsp:open', 'input.text', input.text, { nonEmpty: false });
  ...
  wrapIpcHandler(
    ...
    async (
      ...
      assertString('lsp:change', 'input.text', input.text, { nonEmpty: false });
