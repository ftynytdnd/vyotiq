...
/**
 * Block until `providerId`'s cooldown (if any) has expired. Resolves
 * immediately when the provider is healthy or no cooldown was ever
 * recorded. Aborts cleanly via the optional `signal` so a user-Stop or
 * run abort during a cooldown wait does not leak a dangling timer.
 */
...
/**
 * Test-only helper. Drops every recorded cooldown — used by the
 * vitest suite so each test starts from a clean gate.
 */
