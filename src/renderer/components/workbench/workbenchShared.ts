...
export type CompanionTab = 'editor' | 'terminal' | 'browser' | 'preview' | 'source-control';
...
function sourceControlOpen(): boolean {

  return useSourceControlStore.getState().open;
...
function firstOpenCompanion(): CompanionTab {
  ...
  if (sourceControlOpen()) return 'source-control';
...
export function resolveCompanionTab(tab: WorkbenchTab): CompanionTab {
  ...
  if (tab === 'source-control' && sourceControlOpen()) return 'source-control';
...
export function syncWorkbenchTabAfterClose(): void {
  ...
  const stillValid =
    ...
    (tab === 'source-control' && sourceControlOpen()) ||
...
function listWorkbenchFocusTargets(): WorkbenchFocusTarget[] {
  ...
  if (sourceControlOpen()) targets.push('source-control');
...
function getActiveWorkbenchFocusTarget(): WorkbenchFocusTarget | null {
  ...
  if (tab === 'source-control') return 'source-control';
...
function focusWorkbenchFocusTarget(target: WorkbenchFocusTarget): void {
  ...
  if (target === 'source-control') {

    const sc = useSourceControlStore.getState();

    const workspaceId = sc.workspaceId ?? useWorkspaceStore.getState().activeId;

    if (!sc.open && workspaceId) {

      sc.openPanel(workspaceId);
  ...
  if (target === 'preview') {
    ...
    focusActiveWorkbenchDom('preview');
...
export function closeActiveWorkbenchFocus(): void {
  ...
  if (tab === 'source-control' && sourceControlOpen()) {
