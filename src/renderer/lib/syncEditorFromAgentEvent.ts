/**
 * Keep open editor buffers in sync with agent file-edit / diff-stream events.
 * Invoked from `chatChannel` so we need only one `chat:event` subscription.
 */
...
export function syncEditorFromAgentEvent(event: TimelineEvent): void {
