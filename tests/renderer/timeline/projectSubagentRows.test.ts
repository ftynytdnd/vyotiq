/**
 * projectSubagentRows — collapse consecutive sub-agent lines into V5 batches.
 */
...
describe('projectSubagentRows', () => {
  ...
  it('splits batches when another row interrupts the sub-agent run', () => {
    const rows: Row[] = [
      subLine('A1'),
      subLine('A2'),
    ...
    const out = projectSubagentRows(rows);
    expect(out).toEqual([
      { kind: 'delegate-batch', key: 'delegate:A1:A2', subagentIds: ['A1', 'A2'] },
      { kind: 'tool-group', key: 'tg:1', toolName: 'read', children: [] },
      { kind: 'delegate-batch', key: 'delegate:B1:B2', subagentIds: ['B1', 'B2'] }
    ]);
  });

  it('emits separate batches when assistant-text is interleaved between delegate waves', () => {
    const rows: Row[] = [
      userPrompt(),
      { kind: 'assistant-text', key: 'a1', id: 'msg-turn' },
      subLine('B1'),
    ...
    const out = projectSubagentRows(rows);
    const batches = out.filter((r) => r.kind === 'delegate-batch');
    expect(batches).toHaveLength(2);
    expect(batches[0]).toMatchObject({ subagentIds: ['B1', 'B2'] });
    expect(batches[1]).toMatchObject({ subagentIds: ['C1', 'C2'] });
  });
});
